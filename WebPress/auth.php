...
if(isset($_POST['webpresscreate'])){
	...
	$user = $_POST['webpressuser'];
	$psw = $_POST['webpresspsw'];
	$email = filter_var($_POST['webpressemail'], FILTER_VALIDATE_EMAIL);
	...
	if($isAccepted){
		if(preg_match('/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$/', $email)){
			$email = $email;
				...
				echo '<script>
				window.open("./register?error=invalid_email", "_self");
				</script>';
				return false;
		}
		...
		if(!file_exists(DATA_USERS.'users.dat.json')){
			...
			if(!gettype(file_get_contents(DATA_USERS.'users.dat.json'))||!file_exists(DATA_USERS.'users.dat.json')){
			 ...
			 $data = array($user=>array('name'=>$name, 'psw'=>$psw, 'username'=>$user, 'email'=>$email, 'ip'=>$ip, 'type'=>$type, 'created'=>$date, 'timezone'=>$timezone, 'ban'=>array('reason'=>'', 'time'=>'', 'bannedBy'=>''),'warn'=>array('level'=>0, 'desc'=>''), 'about'=>''));
			 $store = json_encode($data, JSON_PRETTY_PRINT);
			 fwrite($users, $store);
			 fclose($users);
			...
			$date = date('m-d-Y+h:i:sa');
						 $timezone = gettype(Users::ipInfo(Users::getRealIP(), 'timezone'))==="string" ? Users::ipInfo(Users::getRealIP(), 'timezone') : Users::ipInfo(Users::getRealIP(), 'timezone');
			 $store = array('name'=>$name, 'psw'=>$psw, 'username'=>$user, 'email'=>$email, 'ip'=>$ip, 'type'=>$type, 'created'=>$date, 'timezone'=>$timezone, 'ban'=>array('reason'=>'', 'time'=>'', 'bannedBy'=>''),'warn'=>array('level'=>0, 'desc'=>''), 'about'=>'');
			$users[$user] = $store;
			$users = json_encode($users, JSON_PRETTY_PRINT);
			file_put_contents(DATA_USERS.'users.dat.json', $users);
			!CSRF::checkKeyExists() ? CSRF::generate() : CSRF::check();
				echo '<script>
				window.open("./register?success='.$user.'", "_self");
				</script>';
